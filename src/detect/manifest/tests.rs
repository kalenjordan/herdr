...
fn remote_manifest_loads_between_local_override_and_bundled() {
    with_manifest_dirs("remote-source", || {
        write_remote_codex(&remote_manifest("2026.07.23.2", "blocked", "remote-ready"));
        ...
        assert_eq!(explain.manifest_version.as_deref(), Some("2026.07.23.2"));
        assert_eq!(
...
fn detection_uses_cached_manifest_until_explicit_reload() {
    with_manifest_dirs("cache-boundary", || {
        write_remote_codex(&remote_manifest("2026.07.23.2", "blocked", "cached-ready"));
        ...
        write_remote_codex_without_reload(&remote_manifest("2026.07.23.3", "working", "new-ready"));
...
fn codex_osc_working_beats_weak_blocker_screen() {
    // A live [y/n] on screen triggers weak_blocker at priority 600, but an
...
#[test]
fn codex_live_weak_blocker_near_bottom_is_blocked() {
    let screen = "command output\nWould you like to continue? [Y/n]\n";
    let result = osc_explain(Agent::Codex, screen, "llm-proxy", "");
    assert_eq!(result.state, AgentState::Blocked);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
...
#[test]
fn codex_stale_weak_blocker_above_live_prompt_is_idle() {
    let mut screen = String::from("[oh-my-zsh] Would you like to update? [Y/n]\n");
    for line in 1..=13 {
        screen.push_str(&format!("later output {line}\n"));
    }
    screen.push_str("› Implement {feature}\n\n  nudges · gpt-5.6-sol low\n");

    let result = osc_explain(Agent::Codex, &screen, "nudges", "");
    assert_eq!(result.state, AgentState::Idle);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
        Some("osc_title_idle")
