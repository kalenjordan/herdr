...
Add a secret from the foreground machine's text clipboard:

```json
{"id":"req_secret","method":"secret.request","params":{"name":"OPENAI_API_KEY","pane_id":"w1:p1","file":".env","label":"OpenAI API key"}}
```

Herdr resolves the relative environment file from the target pane's workspace
root. The file must be named `.env`, `.env.*`, or `.dev.vars`. The secret value is read
directly from the clipboard and is never included in the request or response.
It must be non-empty, single-line text. The response reports `added` or
`replaced`, plus the variable name and relative file path. This prevents the
secret from entering the agent transcript, but it cannot prevent an agent with
filesystem access from reading the environment file later.
...
Official integrations can also report live context-window usage with `pane.report_agent_context`. This is display-only: it drives the context-percentage status indicator and does not affect waits, notifications, or rollups.

```json
{
  "id": "req_3",
  "method": "pane.report_agent_context",
  "params": {
    "pane_id": "w1:p1",
    "source": "herdr:claude",
    "agent": "claude",
