...
Request a masked prompt in the foreground Herdr client:
...
root. The file must be named `.env` or `.env.*`. The secret value is entered
only in the Herdr modal and is never included in the request or response. The
response reports `added`, `replaced`, or `cancelled`, plus the variable name and
relative file path. This prevents the secret from entering the agent transcript,
but it cannot prevent an agent with filesystem access from reading the
