...
`recent_workspace` opens a most-recently-used workspace overlay. Press the shortcut again while holding Command to cycle, then release Command to switch. `esc` cancels and `enter` switches immediately for terminals that do not report modifier release.

`done_or_blocked_workspace` opens the same switcher filtered to tabs containing a done or blocked agent, ordered oldest-waiting first. Press Command-D again to cycle, then release Command to switch.

`project_picker` opens a typeahead containing open workspaces and immediate subdirectories found under the configured project directories. Selecting an open workspace focuses it. Selecting an unopened directory creates a workspace there and starts the configured command.
