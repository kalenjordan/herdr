...
`secret request` must run inside a Herdr pane. It opens a masked prompt in the
foreground Herdr client, then adds or replaces the variable in the pane
workspace's `.env` file. The value does not pass through the pane, command
arguments, terminal output, or API response. `--file` accepts relative `.env`
or `.env.*` paths only. The requesting command waits until the prompt is saved
or cancelled.
