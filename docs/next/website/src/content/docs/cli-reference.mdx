...
```bash
herdr secret request <VARIABLE_NAME> [--file .env] [--label TEXT]
```

`secret request` must run inside a Herdr pane. Copy the secret value first,
then run the command. Herdr reads the foreground machine's text clipboard and
immediately adds or replaces the variable in the requested environment file.
The value does not pass through the pane, command arguments, terminal output,
or API response. `--file` accepts relative `.env`, `.env.*`, or `.dev.vars`
paths only and defaults to `.env`.

The resulting file remains readable by processes with access to the workspace,
including an agent running as the same operating-system user.
...
```bash
herdr codex rename-thread --current <name>
```

`codex rename-thread` reads the inherited `HERDR_PANE_ID` and submits Codex's
`/rename` command to that pane. It does not accept a pane or thread ID as a
command argument, and it does not rename the Herdr tab.
