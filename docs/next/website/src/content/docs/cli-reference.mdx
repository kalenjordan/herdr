...
immediately adds or replaces the variable in the pane workspace's `.env` file.
The value does not pass through the pane, command arguments, terminal output,
or API response. `--file` accepts relative `.env` or `.env.*` paths only.
...
`codex rename-thread` renames the Codex thread associated with the calling
Herdr pane by submitting Codex's `/rename` command to that pane. It does not
accept a pane or thread ID, and it does not rename the Herdr tab.
